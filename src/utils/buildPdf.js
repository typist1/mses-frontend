...
export function getPdfBlob(resume, { fontScale = 1, margins = 36, lineSpacing = 1.2, bulletStyle = 'dash', sectionOrder: sectionOrderProp } = {}) {
  ...
  function addHeading(text) {
    content.push({
      stack: [
        { text: text.toUpperCase(), bold: true, fontSize: sz(11), margin: [0, sz(8), 0, sz(1)] },
        {
          canvas: [{
  ...
  function entryHeader(main, right) {
    if (!right) {
      return { text: main, bold: true, fontSize: sz(10), margin: [0, sz(4), 0, 0] };
    }
    return {
      columns: [
  ...
  function italicSub(text) {
    return { text, italics: true, fontSize: sz(9), color: GRAY, margin: [0, 0, 0, sz(2)] };
  }
  ...
  for (const key of sectionOrder) {
    ...
    if (key === 'summary' && resume.summary) {
      addHeading('Summary');
      content.push({ text: resume.summary, fontSize: sz(10), margin: [0, 0, 0, sz(3)] });
      continue;
    }
    ...
    if (key === 'skills' && (resume.skills || []).length > 0) {
      ...
      resume.skills.forEach((sk) => {
        ...
        content.push({ text: runs, margin: [0, 0, 0, sz(1.5)] });
      });
      continue;
    }
    ...
    if (key === 'honors_awards' && (resume.honors_awards || []).length > 0) {
      ...
      content.push({ text: runs, margin: [0, 0, 0, sz(3)] });
      continue;
    }
    ...
    if (cs && (cs.title || cs.description)) {
      ...
      if (cs.description) content.push({ text: cs.description, fontSize: sz(10), margin: [0, 0, 0, sz(3)] });
    }
  }
